 ...
 * php.exe -f t2c.php -- -b base -ttable -c
 */
 ...
 * -b base de données
 * -t table
 * -c convertit les noms des champs en camel case 
 */
$options = getopt("b:t:c");  // Récupère les paramètres dans la ligne de commande
//var_dump($options) ; //test
$base=isset($options['b'])? trim($options['b']):'';
$table=isset($options['t'])? trim($options['t']):'*';
$isCamelCase=isset($options['c']);
 ...
 */
  ...
  foreach ($colonnes as $colonne) {
    $data .= PHP_EOL;
    $data .= '  // ' . $colonne['Field'] . PHP_EOL;
    $data .= '  //   ' . $colonne['Type'] . PHP_EOL;
